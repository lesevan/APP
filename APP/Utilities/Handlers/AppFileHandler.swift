...
final class AppFileHandler: NSObject, @unchecked Sendable {
	...
	private func _Zip(download: Download?) throws {
		// 使用ZipArchive解压缩文件
		let success = SSZipArchive.unzipFile(atPath: _ipa.path, toDestination: _uniqueWorkDir.path, overwrite: true, password: nil, progressHandler: { (entry, zipFileInfo, readByte, totalByte) in
