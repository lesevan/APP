...
struct SourcesAddView: View {
	...
	var body: some View {
		NBNavigationView(.localized("添加源"), displayMode: .inline) {
			Form {
				NBSection(.localized("URL链接")) {
					TextField(.localized("输入来源URL"), text: $_sourceURL)
						.keyboardType(.URL)
						.textInputAutocapitalization(.never)
				} footer: {
					Text(.localized("有问题,联系pxx917144686"))
					Text(verbatim: "[\(String.localized("了解更多关于如何设置仓库..."))](https://faq.altstore.io/developers/make-a-source)")
				}
				
				Section {
					Button(.localized("导入"), systemImage: "square.and.arrow.down") {
						_isImporting = true
					...
					Button(.localized("导出"), systemImage: "doc.on.doc") {
						...
						ForEach(_filteredRecommendedSourcesData, id: \.url) { (url, source) in
							HStack(spacing: 2) {
								...
								Button {
									...
									NBButton(.localized("添加"), systemImage: "arrow.down", style: .text)
								}
							}
						}
					} footer: {
						Text(.localized("[提交问题](https://github.com/pxx917144686/APP/issues)。"))
					}
				}
			}
			.toolbar {
					...
					await withCheckedContinuation { continuation in
						dataService.fetch<ASRepository>(from: url) { (result: RepositoryDataHandler) in
							...
							case .failure(let error):
								Logger.misc.error("获取失败 \(url): \(error.localizedDescription)")
							}
							continuation.resume()
						}
					}
