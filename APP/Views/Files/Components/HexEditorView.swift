...
struct HexEditorView: View {
    ...
    var body: some View {
        ...
        .alert("未保存的更改", isPresented: $showingUnsavedAlert) {
            Button("保存") {
    ...
    private var viewModeSegmentedControl: some View {
        Picker("查看模式", selection: $currentViewMode) {
    ...
    private var searchBarView: some View {
        HStack {
            HStack {
                ...
                TextField("搜索十六进制或ASCII", text: $searchText)
...
class HexEditorViewModel: ObservableObject {
    ...
    func loadFile(_ url: URL) {
        print("HexEditorViewModel.loadFile 调用，文件路径: \(url.path)")
        fileURL = url
        isLoading = true
        errorMessage = nil
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    _processFileData()
                    isLoading = false
                    print("HexEditorViewModel: 成功加载 \(hexLines.count) 行")
                }
