...
static void TweakLoaderConstructor() {
    ...
    NSString *globalTweakFolder = nil;
    if (tweakFolderC && tweakFolderC[0] != '\0') {
        globalTweakFolder = [NSString stringWithUTF8String:tweakFolderC];
    }
    ...
    NSArray<NSURL *> *globalTweaks = @[];
    if (globalTweakFolder.length > 0) {
        globalTweaks = [NSFileManager.defaultManager contentsOfDirectoryAtURL:[NSURL fileURLWithPath:globalTweakFolder]
    ...
    // 运行期不加载 .deb/不安装 ElleKit，避免越狱路径依赖与崩溃
    // 若需底层替换，请在打包阶段注入，不在运行时处理。
